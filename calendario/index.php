...
<body>
	<div id="agenda">
		<?php
			require("modelo/mod_connex2.php");
			$conexion = new Connex();
			...
			if (isset($_GET["borrarevento"]))
			{
				...
				if ($r1=mysqli_query($mysqlconn,$q1)) $mostrar="<p class='ok' id='mensaje'>Evento eliminado correctamente.</p>";
			...
			$r1=mysqli_query($mysqlconn,$q1)or die (mysqli_error($mysqlconn));
			if ($f1=mysqli_fetch_array($r1,MYSQLI_ASSOC))
			{
				$h=0;
				do
				{
				...
				while($f1=mysqli_fetch_array($r1,MYSQLI_ASSOC));
			}
			...
			for ($i=1;$i<=$diasdespues;$i++)
			{
				...
				if ($i>=$primeromes && $i<=$tope)
				{
					...
					$revent=mysqli_query($mysqlconn,$sqlevent);
					while($rowevent=mysqli_fetch_array($revent))
					{
